...
export default function DashboardBackground() {
  ...
  useEffect(() => {
    ...
    const ORBS = [
      { px:0.12, py:0.18, r:0.32, vx:0.00010, vy:0.00007, cd:"rgba(30,64,175,0.22)",  cl:"rgba(30,64,175,0.10)" },
      { px:0.80, py:0.55, r:0.26, vx:-0.00008,vy:0.00011, cd:"rgba(6,182,212,0.18)",  cl:"rgba(6,182,212,0.08)" },
      { px:0.45, py:0.88, r:0.22, vx:0.00009, vy:-0.00010,cd:"rgba(79,70,229,0.16)",  cl:"rgba(79,70,229,0.07)" },
      { px:0.90, py:0.12, r:0.18, vx:-0.00012,vy:0.00008, cd:"rgba(6,182,212,0.14)",  cl:"rgba(6,182,212,0.06)" },
      { px:0.25, py:0.65, r:0.20, vx:0.00007, vy:-0.00009,cd:"rgba(30,64,175,0.12)",  cl:"rgba(30,64,175,0.06)" },
      { px:0.60, py:0.30, r:0.15, vx:-0.00006,vy:0.00012, cd:"rgba(6,182,212,0.10)",  cl:"rgba(6,182,212,0.05)" },
    ];
    ...
    const draw = () => {
      ...
      // ── Grid ──
      ctx.strokeStyle = dark ? "rgba(255,255,255,0.028)" : "rgba(15,23,42,0.04)";
      ctx.lineWidth = 0.6;
      const G = 52;
      ...
      LINES.forEach(l=>{
        ...
        const alpha = dark?0.06:0.035;
        ctx.strokeStyle = dark?`rgba(96,165,250,${alpha})`:`rgba(30,64,175,${alpha})`;
        ctx.lineWidth=0.8;
        ctx.beginPath();
      ...
      DOTS.forEach(d=>{
        ...
        ctx.fillStyle=dark?`rgba(96,165,250,${a})`:`rgba(30,64,175,${a})`;
        ctx.beginPath();
