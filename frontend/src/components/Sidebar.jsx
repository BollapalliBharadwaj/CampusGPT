...
function Sidebar({ selectedDocId, onSelectDoc, onDeleteComplete }) {
  const [documents, setDocuments] = useState([]);

  const handleDelete = async (id, name) => {
    if (!window.confirm(`Are you sure you want to delete "${name}"? This will permanently remove the document and all associated text vectors/history.`)) {
      return;
    }
    try {
      ...
      if (onDeleteComplete) {
        ...
        setDocuments(prev => {
          ...
          if (selectedDocId === id) {
            if (next.length > 0) onSelectDoc(next[0]._id);
